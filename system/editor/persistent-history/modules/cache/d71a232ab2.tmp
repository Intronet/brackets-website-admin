/*jslint vars: true, plusplus: true, devel: true, nomen: true, regexp: true, indent: 3 */
...
define(function (require, exports, module) {
   ...
   if(
      prefs.get("pop-up-menu") === undefined || 
      prefs.get("pop-up-menu") === false ||
      brackets.getModule("command/CommandManager").get("rightclickmenu.copy") !== undefined
      ){
      return false;
   } 
   ...
   convertTo_cmenu.openSubMenu = function (mouseOrLocation) {
      ...
      if (mouseOrLocation.target.nodeName === "A") {
         var $window = $(window),
            ...
            posTop = parseInt($(".dropdown.context-menu.open").css("top"), 10) + mouseOrLocation.target.offsetTop,
            posLeft = parseInt($(".dropdown.context-menu.open").css("left"), 10) + mouseOrLocation.target.offsetWidth;
         ...
         if (clip.right > 0) {
            posLeft = Math.max(0, parseInt($(".dropdown.context-menu.open").css("left"), 10) - elementRect.width + 5);
         }
